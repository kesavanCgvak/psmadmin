...
/**
 * Persists flex_integration_logs rows and writes structured debug lines to the stack + flex log files.
 */
class FlexIntegrationLogger
{
    ...
    public function log(
        ...
        FlexIntegrationLog::query()->create([
            ...
            'flex_quote_id' => $flexQuoteId,
            'flex_product_id' => $flexProductId,
        ]);

        $previewLimit = (int) config('flex.log_response_preview_max', 8000);
        ...
        $level = match ($status) {
            FlexIntegrationLog::STATUS_FAILED => 'error',
            FlexIntegrationLog::STATUS_SKIPPED => 'warning',
            FlexIntegrationLog::STATUS_PROCESSING => 'debug',
            default => 'info',
        };

        FlexIntegrationDebugLog::write(
            $level,
