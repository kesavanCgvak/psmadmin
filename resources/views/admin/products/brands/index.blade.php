            ...
            <table id="brandsTable" class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach($brands as $brand)
                        <tr>
                            <td>
                                <input type="checkbox" class="row-checkbox" name="brand_ids[]" value="{{ $brand->id }}"
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            var table = initResponsiveDataTable('brandsTable', {
                "columnDefs": [
            ...
            $('#bulkDeleteBtn').on('click', function() {
                ...
                var message = 'Are you sure you want to delete ' + selectedIds.length + ' brand/brands?\n\n';
                message += 'Brands to be deleted:\n';
                ...
                if (confirm(message)) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                alert('Successfully deleted ' + response.deleted_count + ' brand/brands.');
                                location.reload();
                            } else {
                                alert('Error: ' + (response.message || 'Failed to delete brands.'));
                            }
                        },
                        error: function(xhr) {
                            var message = 'An error occurred while deleting brands.';
